...
impl CleanOptions {
    ...
    /// Create options that remove all outputs and execution state.
    ///
...
impl Notebook {
    ...
    pub fn clean(&self, options: &CleanOptions) -> Notebook {
        let cells = self.cells.iter().map(|cell| clean_cell(cell, options)).collect();
...
fn clean_cell(cell: &Cell, options: &CleanOptions) -> Cell {
    match cell {
        ...
        Cell::Markdown { source, metadata, id } => {
            let new_metadata = clean_cell_metadata(metadata, options);
            let new_id = if options.preserve_cell_ids {
                id.clone()
            } else {
...
fn clean_notebook_metadata(metadata: &NotebookMetadata, options: &CleanOptions) -> NotebookMetadata {
...
mod tests {
    ...
    fn test_clean_original_unchanged() {
        ...
        assert_eq!(notebook.cells[0].outputs().unwrap().len(), original_output_count);
